...
public class AntiMissileLauncher extends AbstractMissileLauncher implements AntiLauncherSystem {


    private final static Texture ANTI_MISSILE_TEXTURE = new Texture("M-45_missile2.png");
    ...
    public AntiMissileLauncher(float height, GameObject owner) {
        ...
        addAmmoType(() -> new AntiMissile(new TextureRegion(ANTI_MISSILE_TEXTURE), 1f, owner));
    ...
    protected void fireInternal(float dt) {
        ...
        // 2f - some empiric fix
        // receive better initial fire angle relative to target
        float maxAcc = currentAmmo.getMaxThrottle() / currentAmmo.getMass() * 2f;
        ...
        targetMissile.get(target).add((AntiMissile) missile);
...
*/
        ...
        missile.getDir().set(dir);

        missile.setTarget(target);

        //System.out.println(gr.impactTime);

        float boost = missile.getFirePower();

        float currBoost = (float) (boost * gr.impactTime/2);
        currBoost = currBoost > boost ? boost : currBoost;
        currBoost = currBoost < boost/3 ? boost/3 : currBoost;
...
//        }
    ...
    */
    ...
    protected GameObject createProjectile() {
        return new AntiMissile(new TextureRegion(ANTI_MISSILE_TEXTURE), 1f, owner);
    }
    */
